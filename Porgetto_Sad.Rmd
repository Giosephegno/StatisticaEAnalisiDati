...
```{r}
p8 <- ggplot(mpg, aes(x = year)) + geom_histogram(bins = 30, aes(y = ..density..), color = "black", fill = "white")
...
```{r}
p7 <- ggplot(mpg, aes(x = year)) + geom_boxplot() + coord_flip()
...
```{r}
p9 <- ggplot(data = mpg, mapping = aes(cyl)) + geom_bar(stat = "count", fill= c("#d62d20", "#ffa700", "#008744","#0057e7")) + ggtitle("Variabile cyl")
