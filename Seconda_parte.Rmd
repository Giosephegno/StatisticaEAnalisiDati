...
Di particolare importanza in statistica è l'*inferenza*. Essa ha lo scopo di estendere le misure ricavate dall'esame di un campione alla popolazione da cui il campione è stato estratto.
...
$$F(x_0) = Pr(X \leq x_0) = \sum_{x \leq x_0} p_i$$
...
$$E(\hat\Theta) = \theta \\Var(\hat \Theta) \leq Var(*)$$
...
$$
\frac{\partial}{\partial \theta} log f(x;\theta) \ esiste \ per \ ogni \ x \ e \ per \ ogni\ \theta \in \Theta \\ E\{ [\frac{\partial}{\partial \theta} logf(X;\theta)]^2 \} \ esiste \ finito \ per \ ogni \ \theta \in \Theta$$
...
$$\lim_{n\to \infty} E(\hat \Theta_n)=\theta \\ \lim_{n \to \infty} Var(\hat \Theta_n)=0$$
...
-   la sua funzione di distribuzione non contiene il parametro $\theta$ da stimare;
...
Alcuni automobilisti sostengono che il consumo medio di carburante in autostrada delle auto sia di $23 \ miglia \ per \ gallone$ altri invece sostengono che sia $17 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.05$ per verificare l'ipotesi nulla $H_0: \mu = 23$ contro l'ipotesi alternativa $H_1: \mu \ne 23$
...
Si nota che $z_{\alpha/2} = 1.959964$ e $z_{os} = -17$ cade al di fuori della regione di accettazione; occorre quindi rifiutare l'ipotesi nulla con un livello di significatività del $5 \%$. Si nota anche che $pvalue < \alpha$ e quindi anche il criterio del p-value consiglia di rifiutare l'ipotesi nulla.
...
Alcuni automobilisti sostengono che la varianza del consumo di carburante in autostrada delle auto sia di $6 \ miglia \ per \ gallone$ altri invece sostengono che sia $4 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.05$ per verificare l'ipotesi nulla $H_0: \mu = 6$ contro l'ipotesi alternativa $H_1: \mu \ne 6$
...
Si nota che $\chi^2_{\alpha/2} = 193$, $\chi^2_{1-\alpha/2} = 278$. Poichè il valore osservato $\chi^2 = 2548$ è compreso nella regione di rifiuto, si rifiuta l'ipotesi nulla con un livello di significatività del $5 \%$.
