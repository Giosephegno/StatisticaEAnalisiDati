***`Il progetto è finito ma è necessario rivedere la definizione di p-value e la definizione del chi quadrato`***
...
La verifica delle ipotesi è un procedimento che consiste nel fare una congettura o un'ipotesi sul parametro non noto $\theta$ o sulla distribuzione di probabilità e nel decidere, sulla base del campione estratto se essa è accettabile o meno. *(approfondiremo questo argomento più avanti)*
...
Si noti che nessun vincolo viene imposto sui valori reali che una v.c può assumere. La rappresentazione grafica più naturale di una v.c. discreta è quella di porre in ascissa i valori $(x_i, x_2,…)$ che la v.c. può assumere ed in ordinata le corrispondenti probabilità $(p_1,p_2,…)$, secondo la Figura seguente:

![](images/download.jpg){width="357"}
...
Tra le variabili casuali discrete rientrano: Bernoulli, Binomiale, Geometrica (geometrica modificata), Binomiale negativa (binomiale negativa modificata), Poisson, Ipergeometrica;
...
![](images/image-286733116.png){width="552"}
...
$$F(x_0) = Pr(X ≤ x_0) = \int^{x_0}_{-\infty} f(w)d_w$$
...
![](images/image-620033264.png){width="326"}
...
```{r}
curve(dnorm(x,mean =0, sd =1), from =-6, to=6, xlab="x", ylab="f(x)", lty =1, main = "Densità normale standard")
...
Per una qualsiasi variabile aleatoria normale $X ∼ N(μ, σ)$ risulta:

$$P(\mu - 3\sigma < X < \mu + 3\sigma) = P(-3 < \frac{X-\mu}{\sigma} < 3)= P(-3 < Z <3)=0.99$$

Quindi la probabilità che una variabile aleatoria $X ∼ N(μ, σ)$ assuma valori in un intervallo avente come centro $μ$ e semiampiezza $3σ$ è prossima all'unità. Questa proprietà delle variabili aleatorie normali è nota come regola del $3σ$.
...
In statistica, un campione casuale è un insieme di elementi scelti in modo casuale da una popolazione. Gli stimatori sono delle funzioni matematiche che utilizzano i dati del campione per fornire informazioni sulla popolazione di origine. Esistono diversi tipi di stimatori, come:
...
Nei metodi di indagine dell'inferenza statistica si considera *un **campione casuale*** $X_1,X_2, ...,X_n$ *di ampiezza* $n$ *se le variabili aleatorie del vettore sono osservabili, indipendenti e identicamente distribuite (iid) con la stessa legge di probabilità della popolazione* (ossia costituiscono delle osservazioni di X) e si cerca di ottenere informazioni sui parametri non noti facendo uso di alcune variabili aleatorie, che sono funzioni misurabili del campione casuale, dette *statistiche* e *stimatori*.

Una **statistica** $t(X_1,X_2, . . . ,X_n)$ è una funzione misurabile e osservabile del campione casuale $X_1,X_2, . . . ,X_n$. Essendo la statistica osservabile, i valori da essa assunti dipendono soltanto dal campione osservato $(x_1, x_2, ..., x_n)$ estratto dalla popolazione e i parametri non noti sono presenti soltanto nella funzione di distribuzione della statistica.
...
Il metodo dei momenti fornisce quindi come stimatore del valore medio $μ$ la media campionaria $\overline X$ e come stimatore della varianza $σ^2$ la variabile aleatoria $\frac{(n − 1)S^2}{n}$.
...
Lo stimatore di massima verosimiglianza e dei momenti del valore medio $μ$ è la media campionaria $\overline X$. Invece lo stimatore di massima verosimiglianza e dei momenti della varianza $σ^2$ è $(n − 1)S^2/n$.
...
**Definizione**:Uno **stimatore** $\hat \Theta= t(X_1,X_2, . . . ,X_n)$ del parametro non noto $\theta$ della popolazione è detto **corretto** (non distorto) se e solo se per ogni $\theta ∈ \Theta$ si ha:
...
Uno stimatore $\hat \Theta$ si dice corretto con varianza uniformemente minima per il parametro non noto $\theta$ se e solo se per ogni $\theta ∈ \Theta$ risulta:

$$E(\hat\Theta) = \theta \\Var(\hat \Theta) ≤ Var(*)$$

per ogni altro stimatore ⋆ corretto del parametro $ϑ$.

###### Disuguaglianza di Cramér--Rao

Sia $\hat \Theta = t(X_1, X_2, ..., X_n)$ uno stimatore corretto del parametro non noto $ϑ$ di una popolazione caratterizzata da funzione di probabilità (nel caso discreto) oppure densità di probabilità (nel caso assolutamente continuo) $f(x; ϑ)$. Se sono soddisfatte le seguenti ipotesi:

$$
\frac{\partial}{\partial \theta} log f(x;\theta) \ esiste \ per \ ogni \ x \ e \ per \ ogni\ \theta ∈ \Theta \\ E\{ [\frac{\partial}{\partial \theta} logf(X;\theta)]^2 \} \ esiste \ finito \ per \ ogni \ \theta ∈ \Theta$$
...
Si desidera verificare che $X$ sia uno stimatore corretto con varianza uniformemente minima del valore medio $E(X) = μ$ di una popolazione normale descritta da una variabile aleatoria $X ∼ N(μ, σ)$ avente varianza nota $\sigma^2$.
...
Uno stimatore $\hat \Theta_n = t(X_1,X_2,...,X_n)$ del parametro non noto $\theta$ della popolazione è detto **consistente** se e solo se per ogni $∈>0$ si ha:

$$lim_{n \to + \inf} P(|\hat \Theta_n - \theta|<∈) =1$$
...
$$lim_{n\to inf} E(\hat \Theta_n)=\theta \\lim_{n \to +inf} Var(\hat \Theta_n)=0$$
...
Un metodo molto utilizzato per la scelta di uno stimatore è invece il **Best Linear Unbiased Estimator (BLUE)**, che consiste nella scelta nella classe degli stimatori lineari corretti di $θ$ di quello che presenta la varianza campionaria minima
...
Fissato un coefficiente di confidenza $1 − α$ $(0 < α < 1)$, è utile scegliere le statistiche $Cn$ e $\overline Cn$ in modo tale che
...
Un metodo per la costruzione degli intervalli di confidenza è il metodo pivotale. Tale metodo consiste essenzialmente nel determinare una variabile aleatoria $\gamma(X_1,X_2,...,X_n;\theta)$ che:

-   dipende dal campione casuale $X_1,X_2, . . . ,X_n$;

-   dipende dal parametro non noto $\theta$ della popolazione;

-   la sua funzione di distribuzione non contiene il parametro $ϑ$ da stimare;

Se sono verificate tali ipotesi allora la variabile può essere interpretata come una variabile aleatoria di pivot
...
Scegliendo $α_1 = −z_{α/2}$ e $α_2 = z_{α/2}$ si ha:
...
Scegliendo $α_1 = −t_{α/2,n−1}$ e $α_2 = t_{α/2,n−1}$ si ha:

$$
P(-t_{\alpha/2,n-1} < T_n < t_{\alpha/2,n-1})
$$

Possiamo dunque dedurre che la stima dell'intervallo di confidenza di grado $1 − α$ per il valore medio $μ$ è:
...
Scegliendo $α_1 = χ^2_{1−α/2,n}$ e $α_2 = χ^2_{α/2,n}$ si ha:
...
Supponiamo che il peso sia distribuito normalmente con valore medio $μ = 23 \ gr$ e varianza non nota $σ^2$, determiniamo una stima dell'intervallo di confidenza di grado $1−α = 0.90$ per la varianza $σ^2$.
...
La stima dell'intervallo di confidenza di grado $1 − α = 0.90$ per la varianza della popolazione normale è quindi (30.69, 41.62).
...
Scegliendo $α_1 = χ^2_{1−α/2,n−1}$ e $α_2 = χ^2_{α/2,n−1}$ si ha:
...
Data la variabile aleatoria che descrive la popolazione $X \sim (\mu, \sigma^2)$, il teorema centrale di convergenza afferma che la variabile:
...
Nell'esempio seguente determiniamo la stima dell'intervallo di confidenza di grado $1−α = 0.90$ della differenza tra i consumi medi $μ1 −μ2$ di carburante delle auto raccolte nel dataset *mpg*.

```{r}
alpha <- 1-0.90
z_1 <- qnorm(1-alpha/2, mean=0, sd=1)
...
La stima dell'intervallo di confidenza di grado $1−α = 0.90$ per la differenza dei consumi medi $μ1 − μ2$ delle auto è $(5.79, 7.37)$; possiamo osservare che il valore risultante della differenza tra le media campionarie rientra nell'intervallo, infatti è pari a: $23.44 - 16.86=6.58$.

Poichè il limite inferiore e il limite superiore sono positivi, si può dedurre che il consumo di carburante in autostrada è più alto rispetto al consumo di carburante in città con un grado di fiducia del 90%.

##### Intervallo di confidenza per $μ1 − μ2$ con varianze non note

Siano $X_1, X_2, ...,X_{n_1}$ e $Y_1, Y_2, ..., Y_{n_2}$ due campioni osservati indipendenti di ampiezza $n_1$ e $n_2$ estratti rispettivamente da due popolazioni normali $X ∼ N(μ_1, σ_1)$ e $Y ∼ N(μ_2, σ_2)$ le cui varianze sono non note. La stima dell'intervallo di confidenza di grado $1 − α$ per la differenza tra le due medie $μ_1 − μ_2$ è:
...
##### **Test su** $μ$ **con varianza** $\sigma^2$ **nota**

**Test bilaterale**

Sia $X_1,X_2, . . .,X_n$ un campione casuale estratto da una popolazione normale descritta da una variabile aleatoria $X ∼ N(μ, σ)$ con varianza nota $σ^2$.
...
Pertanto il test bilaterale $\psi$ di misura $α$ è il seguente:
...
Alcuni automobilisti sostengono che il consumo medio di carburante in autostrada delle auto sia di $23 \ miglia \ per \ gallone$ altri invece sostengono che sia $17 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.05$ per verificare l'ipotesi nulla $H_0: \mu = 23$ contro l'ipotesi alternativa $H_1: \mu ≠ 23$
...
Si nota che $z_{α/2} = 1.959964$ e $z_{os} = -17$ cade al di fuori della regione di accettazione; occorre quindi rifiutare l'ipotesi nulla con un livello di significatività del $5 \%$. Si nota anche che $pvalue < α$ e quindi anche il criterio del p-value consiglia di rifiutare l'ipotesi nulla.

##### **Test su** $μ$ **con varianza non nota**

**Test bilaterale**

Sia $X_1,X_2, . . .,X_n$ un campione casuale estratto da una popolazione normale con varianza non nota $σ^2$.
...
-   si accetti $H_0$ se: $-t_{α/2,n−1} < \frac{\overline x_n - \mu_0}{s_n\sqrt n} < t_{\alpha/2,n-1}$
...
$$
pvalue = P(T_n < −|t_{os}|) + P(T_n > |t_{os}|) = 2 P(T_n > |t_{os}|)= 2[1 − P(T_n ≤ |t_{os}|)]
$$

**Facciamo un esempio di quanto descritto:**

Alcuni automobilisti sostengono che il consumo medio di carburante in città delle auto sia di $17 \ miglia \ per \ gallone$ altri invece sostengono che sia $23 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.01$ per verificare l'ipotesi nulla $H_0: \mu = 17$ contro l'ipotesi alternativa $H_1: \mu ≠ 17$
...
Si nota che $z_{α/2} = 2.597094$ e $z_{os} = 17$ cade al di fuori della regione di accettazione; occorre quindi rifiutare l'ipotesi nulla con un livello di significatività del $1 \%$. Si nota anche che $pvalue < α$ e quindi anche il criterio del p-value consiglia di rifiutare l'ipotesi nulla.

##### **Test su** $\sigma^2$ **con valore medio noto**

**Test bilaterale**

Sia $X_1,X_2, . . . ,X_n$ un campione casuale estratto da una popolazione normale con valore medio noto $μ$.
...
Il test bilaterale $\psi$ di misura $α$ per le ipotesi considerate è il seguente:
...
Alcuni automobilisti sostengono che la varianza del consumo di carburante in autostrada delle auto sia di $6 \ miglia \ per \ gallone$ altri invece sostengono che sia $4 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.05$ per verificare l'ipotesi nulla $H_0: \mu = 6$ contro l'ipotesi alternativa $H_1: \mu ≠ 6$
...
Si nota che $χ^2_{α/2} = 193$, $χ^2_{1-α/2} = 278$. Poichè il valore osservato $χ^2 = 2548$ è compreso nella regione di rifiuto, si rifiuta l'ipotesi nulla con un livello di significatività del $5 \%$.
...
Alcuni automobilisti sostengono che la varianza del consumo di carburante in città delle auto sia di $4 \ miglia \ per \ gallone$ altri invece sostengono che sia $6 \ miglia \ per \ gallone$, pertanto costruiamo un test delle ipotesi di misura $\alpha = 0.05$ per verificare l'ipotesi nulla $H_0: \mu = 4$ contro l'ipotesi alternativa $H_1: \mu ≠ 4$:
...
Si nota che $χ^2_{α/2} = 193$, $χ^2_{1-α/2} = 277$. Poichè il valore osservato $χ^2 = 326$ è compreso nella regione di rifiuto, si rifiuta l'ipotesi nulla con un livello di significatività del $5 \%$.

#### **Test statistici per grandi campioni**
...
$$Z_n = \frac{\overline X_n - \mu}{\sigma/ \sqrt n} \to^d Z$$

converge in distribuzione ad una variabile normale standard.

##### **Test bilaterale approssimato**

Per campioni numerosi, di misura $\alpha$ le ipotesi sono:

$$H_0 : \mu = \mu_0 \\ H_1 : \mu ≠ \mu_0$$

la statistica test è:

$$Z_n = \frac{\overline X_n - \mu_0}{\sigma_0/ \sqrt n}$$

il test bilaterale $\psi$ di misura $\alpha$ è il seguente:

-   Si accetti $H_0$ se: $-z_{\alpha/2} < \frac{\overline x_n - \mu_0}{\sigma_0/ \sqrt n}< z_{\alpha/2}$

-   Si rifiuti $H_0$ se: $\frac{\overline x_n - \mu_0}{\sigma_0/ \sqrt n}< -z_{\alpha/2}$ oppure $\frac{\overline x_n - \mu_0}{\sigma_0/ \sqrt n} > z_{\alpha/2}$
...
Applicando il test chi--quadrato di misura $α = 0.05$, si desidera verificare se la popolazione da cui proviene il campione può essere descritta da una variabile aleatoria $X$ di densità normale.
...
La distribuzione normale ha due parametri non noti $(μ, σ^2)$ e quindi $k = 2$.Pertanto, la funzione di distribuzione della statistica $Q$ è approssimabile con la funzione di distribuzione chi--quadrato con $r − k − 1 = 2$ gradi di libertà. Occorre quindi calcolare $χ^2_{α/2,2}$ e$χ^2_{1−α/2,2}$ con $α = 0.05$.
